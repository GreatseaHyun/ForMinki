...
// ===== Navigation States =====
const NAV_STATE = {
  SEARCH: "search",       // 목적지 검색
  ROUTE_SELECT: "route",  // 경로 선택
  NAVIGATING: "navigating", // 안내 중
  ARRIVED: "arrived",     // 도착
};
...
const MOCK_NAV = {
  destination: "Sogang University Main Gate",
  totalDistance: "1.2km",
  remainingDistance: "840m",
  remainingTime: "11 min",
  nextTurn: "Right at intersection",
  nextTurnDistance: "120m",
...
const NavArrowIcon = () => (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#1B3A5C" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
// ===== Map Placeholder =====
function MapPlaceholder({ state, progress }) {
  return (
    <div
      style={{
        flex: 1,
    ...
    >
      ...
      {(state === NAV_STATE.NAVIGATING || state === NAV_STATE.ROUTE_SELECT) && (
        ...
        >
          <path
            d="M 80 350 Q 120 280 150 250 L 200 200 Q 230 170 280 150 L 350 100"
            fill="none"
            stroke="#3A7BBF"
            strokeWidth="4"
            strokeLinecap="round"
            strokeDasharray={state === NAV_STATE.ROUTE_SELECT ? "8 6" : "none"}
            opacity="0.6"
          />
      ...
      {/* Current location indicator (search state) */}
      ...
      <div style={{ position: "absolute", bottom: 8, right: 8, fontSize: 9, color: "#AAA", backgroundColor: "rgba(255,255,255,0.7)", padding: "2px 6px", borderRadius: 4 }}>
        Map placeholder
      </div>
    </div>
  );
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundColor: selected ? "#F8FBFF" : "white",
        cursor: "pointer",
    ...
    >
      <div
        style={{
          ...
          height: 40,
          borderRadius: 10,
          backgroundColor: isTransit ? "#E3F2FD" : "#E8F5E9",
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          color: isTransit ? "#2E5E8E" : "#2E8B57",
        }}
      >
      ...
      <div style={{ textAlign: "right" }}>
        <div style={{ fontSize: 18, fontWeight: 700, color: "#1B3A5C" }}>{route.duration}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ textAlign: "center" }}>
      <div style={{ fontSize: 22, fontWeight: 700, color: "#1B3A5C" }}>
        {value}
        {unit && <span style={{ fontSize: 12, fontWeight: 400, color: "#999", marginLeft: 2 }}>{unit}</span>}
      </div>
...
// ===== Main Component =====
export default function AivyNavigation() {
  const [navState, setNavState] = useState(NAV_STATE.SEARCH);
  const [searchText, setSearchText] = useState("");
  const [selectedRoute, setSelectedRoute] = useState(null);
  const [navData, setNavData] = useState(MOCK_NAV);

  // Panel heights per state
  const panelHeight = {
    [NAV_STATE.SEARCH]: 260,
    [NAV_STATE.ROUTE_SELECT]: 340,
    [NAV_STATE.NAVIGATING]: 200,
    [NAV_STATE.ARRIVED]: 320,
  };

  const handleSearch = () => {
    if (searchText.trim()) {
      setNavState(NAV_STATE.ROUTE_SELECT);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <div
          onClick={navState === NAV_STATE.NAVIGATING ? undefined : handleFinish}
          style={{
            width: 36,
            height: 36,
            borderRadius: 10,
      ...
      {/* Map area */}
      <MapPlaceholder state={navState} progress={navData.progress} />

      {/* Bottom Panel */}
      <div
        style={{
          height: panelHeight[navState],
          backgroundColor: "white",
      ...
      >
        ...
        {navState === NAV_STATE.SEARCH && (
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            <div style={{ fontSize: 18, fontWeight: 700, color: "#1B3A5C", marginBottom: 14 }}>
              Where to?
            </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {["Nearby cafe", "Pharmacy", "Subway station"].map((q) => (
                <div
                  key={q}
                  onClick={() => { setSearchText(q); setNavState(NAV_STATE.ROUTE_SELECT); }}
                  style={{
                    padding: "8px 14px",
                    borderRadius: 20,
        ...
        {navState === NAV_STATE.ROUTE_SELECT && (
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            <div style={{ fontSize: 12, color: "#999", marginBottom: 14 }}>
              {MOCK_ROUTES.length} routes found
            </div>
            ...
            <div
              onClick={selectedRoute ? handleStartNavigation : undefined}
              style={{
                padding: "14px 0",
                borderRadius: 12,
                backgroundColor: selectedRoute ? "#1B3A5C" : "#E0E0E0",
                color: selectedRoute ? "white" : "#AAA",
                fontSize: 15,
                fontWeight: 600,
                textAlign: "center",
        ...
        {navState === NAV_STATE.ARRIVED && (
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              <CheckCircle />
              <div style={{ fontSize: 18, fontWeight: 700, color: "#1B3A5C", marginTop: 8 }}>
                You've arrived
              </div>
              <div style={{ fontSize: 13, color: "#888", marginTop: 2 }}>
                {MOCK_ARRIVAL.destination}
              </div>
            </div>
