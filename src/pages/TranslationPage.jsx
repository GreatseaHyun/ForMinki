...
const PlayIcon = ({ size = 20, color = "#3A7BBF" }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill={color} stroke="none"><path d="M8 5v14l11-7z" /></svg>
);
const SwapIcon = () => (
    ...
    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
      {[0, 1, 2].map((i) => (
        <div key={i} style={{
          ...
          borderRadius: "50%", border: `2px solid ${color}`, opacity: 0.15 - i * 0.04,
...
function LangPill({ code, name, active }) {
  return (
    <div style={{
      ...
      color: active ? "white" : "#888", fontSize: 13, fontWeight: 600,
      display: "flex", alignItems: "center", gap: 6,
    }}>
      <span style={{ fontSize: 11, opacity: 0.7 }}>{code}</span>{name}
    </div>
  );
}
...
function ChatBubble({ item }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: isMe ? "flex-end" : "flex-start", marginBottom: 14 }}>
      <div style={{ fontSize: 10, color: "#CCC", marginBottom: 3, padding: "0 4px" }}>{isMe ? "You" : "Them"} · {item.time}</div>
      <div style={{
        maxWidth: "85%", padding: "12px 16px",
        borderRadius: isMe ? "16px 16px 4px 16px" : "16px 16px 16px 4px",
        backgroundColor: isMe ? "#1B3A5C" : "#F0F2F5",
      }}>
...
export default function TranslationPage() {
  ...
  if (state === TRANS_STATE.SHOW_CARD && showCardData) {
    return (
      <div style={{ height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: 30, backgroundColor: "#FFFFFF" }}>
        <div style={{ fontSize: 30, fontWeight: 700, color: "#1B3A5C", textAlign: "center", lineHeight: 1.4, marginBottom: 24 }}>
          {showCardData.translation}
        </div>
        <div style={{ width: 60, height: 2, backgroundColor: "#E0E0E0", marginBottom: 24 }} />
        <div style={{ fontSize: 16, color: "#AAA", textAlign: "center", lineHeight: 1.4, marginBottom: 32 }}>
          {showCardData.original}
        </div>
        <div onClick={() => {}} style={{ width: 56, height: 56, borderRadius: "50%", backgroundColor: "#F5F7FA", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", marginBottom: 40, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
          <PlayIcon size={24} color="#1B3A5C" />
        </div>
        <div onClick={() => { setState(TRANS_STATE.IDLE); resetCurrent(); }} style={{ padding: "12px 40px", borderRadius: 12, border: "1.5px solid #EEE", fontSize: 14, color: "#888", cursor: "pointer" }}>
          Close
        </div>
      </div>
    );
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "16px 20px 8px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
        ...
        <div onClick={() => setState(TRANS_STATE.LOG)} style={{ cursor: "pointer", padding: "6px 12px", borderRadius: 8, backgroundColor: "#F0F0F0", display: "flex", alignItems: "center", gap: 6, color: "#888" }}>
          <ListIcon /><span style={{ fontSize: 12, fontWeight: 500 }}>Log</span>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: "0 30px", position: "relative" }}>
        {state === TRANS_STATE.IDLE && (
          <div style={{ textAlign: "center", animation: "fadeUp 0.3s ease" }}>
            <div style={{ width: 80, height: 80, borderRadius: "50%", backgroundColor: "#F0F2F5", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
              <MicIcon size={32} color="#CCC" />
            </div>
            <div style={{ fontSize: 16, fontWeight: 600, color: "#BBB" }}>Ready to translate</div>
            <div style={{ fontSize: 13, color: "#DDD", marginTop: 6 }}>Tap a button below or speak to AIVY</div>
          </div>
        )}
        ...
        {state === TRANS_STATE.LISTENING && (
          <div style={{ textAlign: "center", animation: "fadeUp 0.3s ease", position: "relative" }}>
            <div style={{ position: "relative", width: 160, height: 160, margin: "0 auto 24px" }}>
              ...
              <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 72, height: 72, borderRadius: "50%", backgroundColor: "#FFF3E0", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <MicIcon size={28} color="#E67E22" />
              </div>
            </div>
            <div style={{ fontSize: 12, fontWeight: 600, color: "#E67E22", letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 10 }}>Listening · Japanese</div>
            {currentText && <div style={{ fontSize: 18, fontWeight: 600, color: "#333", lineHeight: 1.5, marginBottom: 8 }}>{currentText}{!currentTranslation && <span style={{ animation: "blink 1s infinite" }}>|</span>}</div>}
            {currentTranslation && <div style={{ fontSize: 15, color: "#3A7BBF", lineHeight: 1.4, fontWeight: 500 }}>→ {currentTranslation}</div>}
          </div>
        )}
        ...
        {state === TRANS_STATE.SPEAKING && (
          <div style={{ textAlign: "center", animation: "fadeUp 0.3s ease", position: "relative" }}>
            <div style={{ position: "relative", width: 160, height: 160, margin: "0 auto 24px" }}>
              ...
              <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 72, height: 72, borderRadius: "50%", backgroundColor: "#E3F2FD", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <MicIcon size={28} color="#3A7BBF" />
              </div>
            </div>
            <div style={{ fontSize: 12, fontWeight: 600, color: "#3A7BBF", letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 10 }}>Your turn · Korean</div>
            {currentText && <div style={{ fontSize: 18, fontWeight: 600, color: "#333", lineHeight: 1.5, marginBottom: 8 }}>{currentText}{!currentTranslation && <span style={{ animation: "blink 1s infinite" }}>|</span>}</div>}
            {currentTranslation && <div style={{ fontSize: 15, color: "#E67E22", lineHeight: 1.4, fontWeight: 500 }}>→ {currentTranslation}</div>}
          </div>
        )}
      </div>
      ...
      <div style={{ padding: "14px 20px 20px", flexShrink: 0 }}>
        {currentTranslation && (state === TRANS_STATE.LISTENING || state === TRANS_STATE.SPEAKING) && (
          <div onClick={handleShowCard} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "12px 0", borderRadius: 12, backgroundColor: "#F0F2F5", marginBottom: 10, cursor: "pointer", color: "#555", fontSize: 13, fontWeight: 500, animation: "fadeUp 0.3s ease" }}>
            <ShowIcon />Show to counterpart
          </div>
        )}
        <div style={{ display: "flex", gap: 10 }}>
          <div onClick={() => { resetCurrent(); setState(state === TRANS_STATE.LISTENING ? TRANS_STATE.IDLE : TRANS_STATE.LISTENING); }}
            style={{ flex: 1, padding: "14px 0", borderRadius: 14, backgroundColor: state === TRANS_STATE.LISTENING ? "#E67E22" : "#FFF3E0", textAlign: "center", cursor: "pointer", transition: "all 0.2s ease" }}>
            ...
            <div style={{ fontSize: 10, color: state === TRANS_STATE.LISTENING ? "rgba(255,255,255,0.7)" : "#E6A756", marginTop: 2 }}>Their language</div>
          </div>
          <div onClick={() => { resetCurrent(); setState(state === TRANS_STATE.SPEAKING ? TRANS_STATE.IDLE : TRANS_STATE.SPEAKING); }}
            style={{ flex: 1, padding: "14px 0", borderRadius: 14, backgroundColor: state === TRANS_STATE.SPEAKING ? "#3A7BBF" : "#E3F2FD", textAlign: "center", cursor: "pointer", transition: "all 0.2s ease" }}>
            ...
            <div style={{ fontSize: 10, color: state === TRANS_STATE.SPEAKING ? "rgba(255,255,255,0.7)" : "#7BAFD6", marginTop: 2 }}>Your language</div>
          </div>
        </div>
      </div>
    </div>
